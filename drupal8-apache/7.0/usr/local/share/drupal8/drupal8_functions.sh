...
#####
do_drupal8_create_directories() {
  mkdir -p /app/${WEB_DIRECTORY}/sites/default/files
}
...
do_drupal8_permissions() {
  if [ "$IS_CHOWN_FORBIDDEN" -ne 0 ]; then
    ...
    chown -R "${APP_USER}":"${APP_GROUP}" /app/${WEB_DIRECTORY}
    chown -R "${APP_USER}":"${APP_GROUP}" /app/config

    # Ensure the files directory is writable.
    chmod -R g+w /app/${WEB_DIRECTORY}/sites/default/files
    ...
    # this later.
    chmod go+w /app/${WEB_DIRECTORY}/sites/default/settings.php
  fi
}

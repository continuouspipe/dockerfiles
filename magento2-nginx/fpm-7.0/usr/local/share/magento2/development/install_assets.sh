...
if [ -f /app/tools/assets/development/media.files.tgz ]; then
  ...
  as_code_owner "tar --no-same-owner --extract --strip-components=2 --touch --overwrite --gzip --file=/app/tools/assets/development/media.files.tgz || exit 1" /app/pub/media

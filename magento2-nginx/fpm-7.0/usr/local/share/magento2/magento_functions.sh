  ...
  if [ -n "$AWS_S3_BUCKET" ]; then
    for asset_env in $ASSET_DOWNLOAD_ENVIRONMENTS; do
      if [ -n "$ASSET_DOWNLOAD_EXCLUDE_PATTERN" ]; then
        as_build "aws s3 sync 's3://${AWS_S3_BUCKET}/${asset_env}' 'tools/assets/${asset_env}' --exclude='${ASSET_DOWNLOAD_EXCLUDE_PATTERN}' --recursive"
      else
        as_build "aws s3 sync 's3://${AWS_S3_BUCKET}/${asset_env}' 'tools/assets/${asset_env}' --recursive"
      fi
    done
  fi
}
